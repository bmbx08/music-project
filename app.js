...
const getAlbums= async (limit)=> {
  const response = await fetch(`https://api.spotify.com/v1/search?q=${keyword}&type=album,artist,playlist,track&market=KR`, {
    method: 'GET',
    headers: { 'Authorization': 'Bearer ' + currentToken.access_token },
  });
  const albumsList = await response.json();
  artistsAlbumsList = albumsList.albums.items
  console.log("Albums",artistsAlbumsList);  
  renderAlbums();
}


const renderAlbums=()=>{
  const AlbumsHTML = artistsAlbumsList.map(
    (album) =>
      `
  ...
  console.log("HTML: ",AlbumsHTML)
  document.getElementById("main-content").innerHTML = AlbumsHTML;
}